...
bano.a2a <- function(df, relative = FALSE) {
  ...
  #  for(i in (1:nrow(df))){
  #    currentActivity <- df$Activity[i]
  #    for(j in (2:ncol(df))){
  #      df[df$Activity == currentActivity, j] <- df[df$Activity == currentActivity, j] / accessCount[accessCount$Activity == currentActivity, 2] 
  #    }
  #  }
  #  df
    df %>%
...
permits.a2a <- bano.a2a(permits.union)

permits.a2a.rel <- bano.a2a(Permits, relative = TRUE)
